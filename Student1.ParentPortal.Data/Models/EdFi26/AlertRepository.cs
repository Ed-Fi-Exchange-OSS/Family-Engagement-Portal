...
namespace Student1.ParentPortal.Data.Models.EdFi26
{
    public class AlertRepository : IAlertRepository
    {
        ...
        public AlertRepository(EdFi26Context edFiDb)
        {
            _edFiDb = edFiDb;
        }
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdAbsence(decimal excusedThreshold, decimal unexcusedThreshold, decimal tardyThreshold, string excusedDescriptor, string unexcusedDescriptor, string tardyDescriptor)
        {
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               ...
                                               where g.Where(x => x.ssae.AttendanceEventCategoryDescriptor.Descriptor.CodeValue == tardyDescriptor).Count() >= tardyThreshold
                                                    || g.Where(x => x.ssae.AttendanceEventCategoryDescriptor.Descriptor.CodeValue == excusedDescriptor).Count() >= excusedThreshold
                                               ...
                                               select new
                                               {
                                                   ...
                                                   excusedCount = g.Where(x => x.ssae.AttendanceEventCategoryDescriptorId == 102).Count(),
                                                   unexcusedCount = g.Where(x => x.ssae.AttendanceEventCategoryDescriptorId == 98).Count(),
                                                   tardyCount = g.Where(x => x.ssae.AttendanceEventCategoryDescriptorId == 100).Count(),
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdAssignment(decimal assignmentThreshold, string[] gradeBookMissingAssignmentTypeDescriptors, string missingAssignmentLetterGrade)
        {
            ...
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               ...
                                               join sge in _edFiDb.StudentGradebookEntries
                                                            .Include(x => x.GradebookEntry.GradebookEntryType)
        ...
        public async Task<List<StudentAlertModel>> studentsOverThresholdCourse(decimal courseThreshold, string gradeTypeGradingPeriodDescriptor)
        {
            ...
            var studentsOverThreshold = await (from s in _edFiDb.Students
                                               ...
                                               where pa.AlertsEnabled
                                                       && sy.CurrentSchoolYear && gra.NumericGradeEarned.HasValue
                                                       && gra.NumericGradeEarned.Value <= courseThreshold
                                                       && gra.GradeType.CodeValue == gradeTypeGradingPeriodDescriptor
                                               group new { s, co, gra, profile, pa, spa } by s.StudentUsi into g
                                               select new
                                               {
                                                   StudentUSI = g.Key,
                                                   Data = g.GroupBy(x => x.pa.ParentUniqueId).Select(data => new StudentParentsAlertsProfilesModel
